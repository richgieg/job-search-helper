...
The bootstrap hydrate has now been removed from the app shell, and the legacy `status.hydration` and `hydrate()` store scaffolding have been removed as well. Runtime navigation no longer depends on a startup whole-app fetch.
